...
import com.modal.complaintManagementSystem.configuration.MailConfiguration;
...
public class MailSending {
	...
	public boolean SendEmailRgisterSuccessfully(UserDto userDto) {
		JavaMailSender mailSender = mailConfiguration.mailConfig();
		SimpleMailMessage message = new SimpleMailMessage();
		message.setFrom("xworkzvendormanagement@gmail.com"); // Set your email here
		message.setTo(userDto.getEmail());
		message.setSubject("Thank you for Registering");
		message.setText("Dear " + userDto.getFirstName() + ",\n\n"
		        + "Welcome to our community!\n\n"
		        + "We are committed to ensuring your satisfaction and addressing any concerns you may have. At the heart of our operations lies our complaint management system, designed to resolve issues swiftly and effectively.\n\n"
		        + "The complaint management process is crucial for us to maintain the highest standards of service. Whether it's an issue with a product, service, or any aspect of your experience, our team is dedicated to resolving it promptly.\n\n"
		        + "Here's how our complaint management system works:\n"
		        + "- You can submit a complaint through our website or customer service channels.\n"
		        + "- Each complaint is assigned a unique reference number for easy tracking.\n"
		        + "- Our team will investigate your complaint thoroughly and provide a resolution within the specified timeframe.\n"
		        + "- We prioritize transparency throughout the process, keeping you informed of the status and progress of your complaint.\n\n"
		        + "Your security and satisfaction are paramount to us. We handle all complaints with the utmost confidentiality and professionalism.\n\n"
		        + "Your account is currently being processed. Once verified, you will receive a confirmation email and gain access to our complaint submission portal.\n\n"
		        + "Below are your registration details:\n"
		        + "- Owner Name: " + userDto.getFirstName() + "\n"
		        + "- Email: " + userDto.getEmail() + "\n"
		        + "- Contact Number: " + userDto.getContactNumber() + "\n"
		        + "- Alternate Contact Number: " + userDto.getAlterContactNumber() + "\n"
		        + "- Address: " + userDto.getAddress() + "\n\n"
		        + "We look forward to serving you and resolving any concerns you may have. Your feedback is invaluable as we strive to improve our services.\n\n"
		        + "Thank you for choosing us. We are here to ensure your experience with us is nothing short of excellent.\n\n"
		        + "Best regards,\n"
	 ...
	 * message.setSubject("Thank you for Registering"); message.setText("Dear " +
	 * vendorDTO.getOwnerName() + ",\n\n" + "Welcome to our community!\n\n" +
	 ...
	 * e) { e.printStackTrace(); return false; } } public boolean
	 * SendEmailAccountVerified(VendorDTO vendorDTO) { JavaMailSender mailSender =
	 * mailConfiguration.mailConfig(); SimpleMailMessage message = new
	 * SimpleMailMessage(); message.setFrom("xworkzvendormanagement@gmail.com"); //
	 * Set your email here message.setTo(vendorDTO.getEmailId());
	 ...
	 * 
	 * 
	 * //// send Login OTP To Email Method public boolean sendLoginOTPToEmail(String
	 * email, String OTP) { JavaMailSender mailSender =
	 * mailConfiguration.mailConfig(); SimpleMailMessage message = new
	 * SimpleMailMessage(); message.setFrom("xworkzvendormanagement@gmail.com"); //
	 * Set your email here message.setTo(email); message.setSubject("Login OTP");
	 * message.setText("Dear User,\n\n" + "Your OTP for login is: " + OTP + "\n\n" +
	 * "Please use this OTP to log in to your account.\n\n" + "Best regards,\n" +
	 * "xworkz vendor management Team");
	 * 
	 * try { mailSender.send(message); // Use mailSender to send the email
	 * System.out.println("login  OTP Mail sent successfully"); return true; } catch
	 * (Exception e) { e.printStackTrace(); return false; } }
	 * 
	 * public boolean sendOrderInvoiceToAdmin(String email, String orderStatus,
	 * String productName, MultipartFile file) { JavaMailSender mailSender =
	 * mailConfiguration.mailConfig(); MimeMessage message =
	 * mailSender.createMimeMessage(); try { MimeMessageHelper helper = new
	 * MimeMessageHelper(message, true); helper.setFrom(email); // Set sender email
	 * helper.setTo("xworkzvendormanagement@gmail.com");
	 * helper.setSubject("Order Invoice");
	 * 
	 * // Prepare email body String emailBody = "Dear Admin,\n\n"; emailBody +=
	 * "An order invoice has been generated for the product: " + productName +
	 * ".\n\n"; emailBody += "Order Status: " + orderStatus + ".\n\n"; emailBody +=
	 * "Please find the attached invoice.\n\n"; emailBody += "Best regards,\n";
	 * emailBody += "xworkz vendor management Team";
	 * 
	 * // Set email body helper.setText(emailBody);
	 * 
	 * // Attach file if (file != null && !file.isEmpty()) {
	 * helper.addAttachment(Objects.requireNonNull(file.getOriginalFilename()), new
	 * ByteArrayResource(file.getBytes())); }
	 * 
	 * // Send email mailSender.send(message);
	 * System.out.println("Order invoice email sent successfully"); return true; }
	 ...
	 * SimpleMailMessage message = new SimpleMailMessage();
	 * message.setFrom("xworkzvendormanagement@gmail.com"); // Set your email here
	 * message.setTo(email); message.setSubject("admin login  OTP");
	 ...
	 * 
	 * try { mailSender.send(message); // Use mailSender to send the email
	 * System.out.println("OTP Mail sent successfully"); return true; } catch
	 * (Exception e) { e.printStackTrace(); return false; } }
	 * 
	 * public boolean sendMessage(String senderEmail, String message) {
	 * JavaMailSender mailSender = mailConfiguration.mailConfig(); SimpleMailMessage
	 * mailMessage = new SimpleMailMessage(); mailMessage.setFrom(senderEmail); //
	 * Set your email here mailMessage.setTo("xworkzvendormanagement@gmail.com");
	 * mailMessage.setSubject("Message from Vendor Management"); String
	 * emailAndMessage = "Sender's Email: " + senderEmail + "\n\n" + message;
	 * mailMessage.setText(emailAndMessage);
	 * 
	 * try { mailSender.send(mailMessage);
	 * System.out.println("Email sent successfully"); return true; } catch
	 * (Exception e) { e.printStackTrace(); return false; } }
	 * 
	 * public boolean sendCustomEmail(String recipientName, String recipientEmail,
	 * String subject, String emailContent) { JavaMailSender mailSender =
	 * mailConfiguration.mailConfig(); SimpleMailMessage message = new
	 * SimpleMailMessage(); message.setFrom("xworkzvendormanagement@gmail.com"); //
	 * Set your email here message.setTo(recipientEmail);
	 * message.setSubject(subject); message.setText("Dear " + recipientName +
	 * ",\n\n" + emailContent + "\n\n" + "Best regards,\n" +
	 * "Xworkz Vendor Management Team");
	 * 
	 * try { mailSender.send(message); // Use mailSender to send the email
	 * System.out.println("Custom email sent successfully"); return true; } catch
	 * (Exception e) { e.printStackTrace(); return false; } }
	 */
